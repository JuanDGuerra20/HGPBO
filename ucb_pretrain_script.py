...
kern_op = 'add_kernel'
folder_of_the_day = (str(workspace_folder) + f'/ucb_pretrain/data-' + str(name_code) + str(current_dateday))
...
def joint_plots(joint_exploit, joint_explor, k_vals, folder_of_the_day, nbr_query, nbr_repetition):
    ...
    plt.savefig(
        f'ucb_pretrain{folder_of_the_day}/differentiable_plots/Joint_UCB_Interval_Neural_Propagation_HGPBO_{nbr_repetition}_Exploitation_query_{nbr_query}')
    plt.close()
    ...
    plt.savefig(
        f'ucb_pretrain{folder_of_the_day}/differentiable_plots/Joint_UCB_Interval_Neural_Propagation_HGPBO_{nbr_repetition}_Exploration_query_{nbr_query}')
    plt.close()


def select_next_query(qi, x, y):
    p_x = x[20 * qi: 20 * (qi + 1)]
    p_y = y[20 * qi: 20 * (qi + 1)]

    rand_choice = np.random.randint(0, len(p_x))

    next_query = p_x[rand_choice]
    next_value = p_y[rand_choice]
    return next_query, next_value

def train_submodels(q_pre, x1, x2, y1, y2, test_x_1D):
    ...
    for q in range(q_pre):
        ...
        q_x1, q_y1 = select_next_query(next_query1, x1, y1)
        q_x2, q_y2 = select_next_query(next_query2, x2, y2)
    ...
    vi.contour_plot_1D([sub1, sub2], test_x_1D, [test_y_1D / torch.max(test_y_1D), test_y_1D / torch.max(test_y_1D)],
                       f'',"ucb_pretrain", folder_of_the_day, 'N.A', neural=True, save=False)
...
def training_procedure(nbr_query, nbr_repetition, nbr_rand_init, training_iter, k_vals):
    ...
    for kappa in k_vals:
        ...
        for repetition in range(nbr_repetition):
            ...
            max_seen_resp_1_1D = 0
            max_seen_resp_2_1D = 0
            sub1_qc = torch.zeros(test_x_1D.shape)
            sub2_qc = torch.zeros(test_x_1D.shape)
            sub2_qc = torch.zeros(test_x_hier.shape)

            for q in tqdm(range(nbr_query)):
                if q == 0:
                    sub1, sub1_like, sub2, sub2_like, train_x_sub1, train_y_sub1, train_x_sub2, train_y_sub2, max_seen_resp_1_1D, max_seen_resp_2_1D = train_submodels(20, x_sub1, x_sub2, y_sub1, y_sub2, test_x_1D)
                    ...
                    # TODO: Make the contribution here too
        ...
        plt.savefig(
            f'ucb_pretrain{folder_of_the_day}/differentiable_plots/UCB_Interval_Efficient_Prop_Neural_HGP-BO_{nbr_repetition}_repetitions_kappa_{k}_query_{nbr_query}')
        plt.close()


        vi.contour_plot_1D([sub1, sub2], test_x_1D, [test_y_1D / torch.max(test_y_1D), test_y_1D / torch.max(test_y_1D)],
                         f'/Heatmap_Neural_UCB_INTERVAL_Efficient_HGP-BO_{nbr_repetition}_repetitions_kappa_{k}_query_{nbr_query}',
                         "ucb_pretrain", folder_of_the_day, 'N.A', neural=True)
