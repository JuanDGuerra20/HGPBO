...
def run_repetition(kappa, gamma, nu, nbr_query, nbr_rand_init, dimension, training_iter, hierarchical_model, data_creation_func, eps, model_name, folder_of_the_day, data_name, final=False, children=[], seed=True, noise=0.1, alpha=0.75):
    ...
    for q in tqdm(range(nbr_query)):
        if q == 0:
            ...
            train_x_hier, train_y_hier = test_x_hier[next_query_pins].reshape((1, 3)), y_hier.flatten()[
                next_query_pins].reshape((1,))
...
def training_procedure(nbr_query, nbr_repetition, nbr_rand_init, dimension, training_iter, k_vals, g_vals, nu_vals, data_name, data_creation_func,
                           eps, hierarchical_model, multi, seed, children=[], visualize=True, noise=0.1, alpha=0.75):
        ...
        for gamma in g_vals:
            for nu in nu_vals:
                ...
                vi.contour_plot_1D(master.sub_models, x_sub1, [y_sub1 / torch.max(y_sub1), y_sub2 / torch.max(y_sub2), y_sub3 / torch.max(y_sub3)],
                                   f'/contour/Contour_{data_name}_HGP-BO_{nbr_repetition}_repetitions_dim_{dimension}_kappa_{k}_gamma_{g}_nu_{n}_a_{a}',
                                   f"{model_name}", folder_of_the_day, data_name, parent=master)
                ...
                plt.title(
                    f'{model_name} HGP-BO {nbr_repetition} repetitions with kappa {k} Gamma {g} Nu {n} Init {nbr_rand_init} Alpha {alpha}')
                plt.savefig(
                    f'{data_name}/{model_name.lower()}{folder_of_the_day}/differentiable_plots/BIF_{data_name}_HGPBO_{nbr_repetition}_repetitions_init_{nbr_rand_init}_training_iter_{training_iter}_kappa_{k}_gamma_{g}_nu_{n}_a_{a}.svg')
                plt.savefig(
                    f'{data_name}/{model_name.lower()}{folder_of_the_day}/png/BIF_{data_name}_HGPBO_{nbr_repetition}_repetitions_init_{nbr_rand_init}_training_iter_{training_iter}_kappa_{k}_gamma_{g}_nu_{n}_a_{a}.png')
                ...
                df.to_csv(
                    f'{data_name}/{model_name.lower()}{folder_of_the_day}/csv/kappa_{k}_gamma_{g}_nu_{n}_model_state_{nbr_query}_queries_init_{nbr_rand_init}_train_iter_{training_iter}_repetitions_{nbr_repetition}_a_{a}')
        ...
        for dataset_num in [6]:
            ...
            name, master, better_exploration_score, better_exploitation_score, r2, child_1_r2, child_2_r2, child_2_r2 = \
                training_procedure(nbr_query, nbr_repetition, nbr_rand_init, dimension, training_iter, k_vals,
                                   g_vals, nu_vals, data_name, data_creation_func, eps, h, multi, seed, noise=0.1, alpha=a)[0]
