...
class Hierarchical_GP(gpytorch.models.ExactGP):
    ...
    def Hoptimize(model, likelihood, training_iter, train_x, train_y, verbose=True):
        ...
        for sub in model.sub_models:
            sub.train()
            sub_optimizers.append(torch.optim.Adam(sub.parameters(), lr=0.1))
...
def update_model1_1D_Efficient(model, likelihood, train_x, train_y, next_query_pin, response1, training_iter=10):
    """
    Used to update the submodel within the hierarchical model
    :param model: the child model that will be updated
    :param likelihood: the likelihood associated to param model
...
def update_model1_1D_max_seen(model, likelihood, train_x, train_y, next_query_pin, response1, env, training_iter=10):
    ...
    # Update training data by adding next_query_value to the train dataset
    train_x, train_y = model.update_training_data(train_x, train_y, next_query_pin, response1,
                                                  env)  # has to be 1D dataset
    # Update the model with the new training data
...
def decompose_hkernel_loss(model, loss, train_x):
    ...
    for i in range(len(train_x)):
        ...
        p2 = (math.sqrt(2 * c2.outputscale) * math.sqrt(math.dist(train[0, 2:], train[1, 2:])) / c2.lengthscale) ** (-c2.nu)
        kv2 = p1 * p2
...
def update_model1_1D_Efficient_MAX(model, likelihood, train_x, train_y, next_query_pin, response1, max_seen, training_iter=10):
    """
    Used to update the submodel within the hierarchical model
    :param model: the child model that will be updated
    :param likelihood: the likelihood associated to param model
    ...
    train_x, train_y = update_training_data(train_x, train_y, next_query_pin, response1)  # has to be 1D dataset
    # Update the model with the new training data
    model.set_train_data(train_x, train_y/max_seen, strict=False)
    # Find optimal model hyperparameters
    model.train()
    likelihood.train()

    model, likelihood = optimize(model, likelihood, training_iter, train_x, train_y/max_seen, verbose=False)
