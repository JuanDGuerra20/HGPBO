...
if __name__ == "__main__":
    ...
    for d in datasets:
        for m in models:
            if m == "vanilla":
                files = glob.glob(f"{d}/{m}/data-2025-11-27/csv/kappa_7,5_model_state_100_queries_init_1_train_iter_10_repetitions_*")
                #get_ordered_scores(files, "kappa", m, d)
            else:
                files = glob.glob(f"{d}/{m}/data-2025-11-27/csv/*_rep_init_*_train_iter_10_eps_*_k_7,5_g_3,5_nu_0_5_noise_0,1.csv")
