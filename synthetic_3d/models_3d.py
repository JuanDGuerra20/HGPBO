...
def get_next_query_value(next_query_pins, X, Y, nbr_rdm_points_data=20, noise=0):
    ...
    for indices, pins in enumerate(X):
        ...
        if pins[0] == next_query_pins[0] and pins[1] == next_query_pins[1] and pins[2] == next_query_pins[2]:
...
def get_exploration_score(y_mu, ground_truth_max, coord_pins, X, Y, nbr_rdm_points_data=20):
    ...
    for indices_x, pins in enumerate(X):
        ...
        for indices_y, sub_pin in enumerate(pins):
            for indices_z, sub_pin2 in enumerate(sub_pin):
                if sub_pin2[0] == next_query_pins[0] and sub_pin2[1] == next_query_pins[1] and sub_pin2[2] == next_query_pins[2]:
                    new_training_values_tampon[i] = Y[indices_x, indices_y, indices_z]
                    i += 1
...
def get_instantaneous_regret(y_mu, ground_truth_max, coord_pins, X, Y, nbr_rdm_points_data=20):
    ...
    for indices_x, pins in enumerate(X):
        ...
        for indices_y, sub_pin in enumerate(pins):
            for indices_z, sub_pin2 in enumerate(sub_pin):
                if sub_pin2[0] == next_query_pins[0] and sub_pin2[1] == next_query_pins[1] and sub_pin2[2] == \
                        next_query_pins[2]:
                    new_training_values_tampon[i] = Y[indices_x, indices_y, indices_z]
                    i += 1
