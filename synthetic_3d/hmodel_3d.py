...
class PriorMean(gpytorch.means.Mean):  # PMF
        ...
        for i in range(len(test_x)):
            for j in range(len(test_x)):
                for k in range(len(test_x)):
                    self.hash_map.set_val(str(test_x[i][j][k]), [i,j,k])

    """def forward(self, input):

        Xmean_1D = torch.linspace(0.0, 4.0, self.map.shape[0]).double()
        Xmean_2D = torch.zeros((self.map.shape[0], self.map.shape[1], self.map.shape[2], 3)).double()

        for i in range(len(Xmean_1D)):
            for j in range(len(Xmean_1D)):
                for k in range(len(Xmean_1D)):
                    Xmean_2D[i, j, k, 0] = Xmean_1D[i]
                    Xmean_2D[i, j, k, 1] = Xmean_1D[j]
                    Xmean_2D[i, j, k, 2] = Xmean_1D[k]
        ...
        for i in range(input.shape[0]):
            ...
            while flag:
                ...
                while flag and y < Xmean_2D.shape[1]:
                    z = 0
                    while flag and z < Xmean_2D.shape[2]:
                        if Xmean_2D[x][y][z][0] == input[i][0] and Xmean_2D[x][y][z][1] == input[i][1] and Xmean_2D[x][y][z][2] == input[i][2]:
                            indice_1 = x
                            indice_2 = y
                            indice_3 = z
                            flag = False
            ...
            new_prior[i] = self.map[indice_1, indice_2, indice_3]
    ...
    def forward(self, input):
        ...
        for i in range(input.shape[0]):
            ...
            new_prior[i] = self.map[look_up[0], look_up[1], look_up[2]]

        return new_prior

    #def bin_search(self, input, Xmean_2D):
...
class Efficient_UCB_Hierarchical_GP(gpytorch.models.ExactGP):
    ...
    def simplified_subkernel_loss_sanity(self, loss, train_x, train_y, sub_optimizers, sub_loss_functions):
        ...
        for i, sub in enumerate(self.sub_models):
            ...
            sub.train()
            sub_out = sub(sub.train_inputs[0])  # re-evaluates model on training inputs to get a fresh output for MLL computation

            loss_func = sub_loss_functions[i]
            sub_opt = sub_optimizers[i]

            sub_loss = -loss_func(sub_out, sub.train_targets)   # negated to match the minimization convention of the optimizer
        ...
        for x in range(len(domain)):
            for y in range(len(domain[x])):
                for z in range(len(domain[x][y])):
                    if domain[x][y][z][0] == query[0] and domain[x][y][z][1] == query[1] and domain[x][y][z][2] == query[2]:
                        query_c[x*(len(domain[x]) * len(domain[x][y])) + y*len(domain[x]) + z] += 1
                        break
...
def hierarchical_kernel(kernel_type, submodels):
    """
    ...
    for i, sub in enumerate(submodels):
        kernels.append(gpytorch.kernels.MaternKernel(nu=2.5, has_lengthscale=True, ard_num_dims=1,
                                                   input_dim=len(submodels), active_dims=[i]))
        kernels[i].outputscale = sub.covar_module.outputscale
    ...
    if kernel_type == 'add_kernel':
        ...
        for i, kernel in enumerate(kernels):
            if i == 0:
                ...
                hkernel = hkernel + kernel

    return hkernel


def update_kernel_parameters(model, submodels):
    """
    ...
    for i, sub in enumerate(submodels):
        model.covar_module.kernels[i].outputscale = sub.covar_module.outputscale
        model.covar_module.kernels[i].lengthscale = sub.covar_module.base_kernel.lengthscale
