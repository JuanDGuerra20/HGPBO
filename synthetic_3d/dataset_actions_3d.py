...
class Trains:
    ...
    def build_f_grid(self, emg=2, syn=None, dt=DT, f='meanmax'):
        ...
        for i, ch1 in enumerate(self.chs):
            for j, ch2 in enumerate(self.chs):
                # synergy mode currently only supports f='meanmax'
...
def random_initialization_1D(random_sample, trainsC, max_seen_resp, dt=DT, emg=EMG, max_update=False):
    ...
    for _ in range(random_sample):
        ...
        # normalization applied here; validate if extending to new dataset types
...
def count_pin(selected_pin_array, x):
    # behaviour under HGP-BO may differ; validate when extending
    """
...
def create_data_storage(nbr_query, nbr_repetition, nbr_pins):
    # returns a tuple; consider a dict API if extending
...
def hierarchical_select_random_queries(num_queries, x, y, seed=False, noise=0):
    ...
    indices_Z = np.random.randint(len(x), size=(num_queries))


    train_x = x[indices_X, indices_Y, indices_Z]
    train_y = y[indices_X, indices_Y, indices_Z]
    train_y += np.random.normal(0, noise*(torch.max(y)-torch.min(y)), size=train_y.shape)
    ...
    for i in range(len(y_sub1)):
        for j in range(len(y_sub2)):
            for k in range(len(y_sub3)):
                y_hier[i, j, k] = (y_sub1[i] + y_sub2[j] + y_sub3[k])**2 / (
                    (torch.sqrt((x_sub1[i] - x_sub2[j])** 2 + (x_sub1[i] - x_sub3[k])** 2 + (x_sub2[j] - x_sub3[k])** 2)  + eps))
...
def generate_3d_2_dataset(dimension, eps):
    ...
    for i in range(len(y_sub1)):
        for j in range(len(y_sub2)):
            for k in range(len(y_sub3)):
                y_hier[i, j, k] = (y_sub1[i] + y_sub2[j] + y_sub3[k])**2 + 2

    y_hier = y_hier.double()

    return x_sub1, y_sub1, x_sub2, y_sub2, x_sub3, y_sub3, x_hier, y_hier, test_x, test_x_hier

def generate_3d_3_dataset(dimension, eps):
    x_sub1 = torch.linspace(0, 4, dimension).double()
    ...
    for i in range(len(y_sub1)):
        for j in range(len(y_sub2)):
            for k in range(len(y_sub3)):
                y_hier[i, j, k] = (y_sub1[i] + y_sub2[j] + y_sub3[k])/ (
                    (torch.sqrt(((x_sub1[i] - x_sub2[j])** 2 + (x_sub1[i] - x_sub3[k])** 2 + (x_sub2[j] - x_sub3[k])** 2)) + eps))
...
if __name__ == '__main__':
    x_sub1, y_sub1, x_sub2, y_sub2, x_sub3, y_sub3, x_hier, y_hier, test_x, test_x_hier = generate_3d_dataset(10, 4)
