...
workspace_folder = (r'C:\Users\preda\PycharmProjects\HPGBO') #path to folder

os.chdir(workspace_folder)

kern_op = 'add_kernel'
folder_of_the_day = (str(workspace_folder) + f'/ucb_max_efficient/data-' + str(name_code) + str(current_dateday))


def joint_plots(joint_exploit, joint_explor, kappa, g_vals, folder_of_the_day, nbr_query, nbr_repetition):

    for i, gamma in enumerate(g_vals):
        plt.plot(joint_exploit[i], label=f'K {kappa}, G {gamma}')

    plt.legend()
    plt.xlabel(f'Nbr Queries')
    plt.ylim((0, 1.1))
    plt.ylabel(f'Exploitation Score')
    plt.title(f'Joint UCB_MAX_Efficient Propagation HGPBO {nbr_repetition} Exploitation')
    plt.savefig(
        f'ucb_max_efficient{folder_of_the_day}/differentiable_plots/Joint_UCB_Efficient_Neural_Propagation_HGPBO_{nbr_repetition}_Exploitation_kappa_{kappa}_query_{nbr_query}')
    plt.close()

    for i, gamma in enumerate(g_vals):
        plt.plot(joint_explor[i], label=f'K {kappa}, G {gamma}')

    plt.legend()
    plt.xlabel(f'Nbr Queries')
    plt.ylim((0, 1.1))
    plt.ylabel(f'Exploration Score')
    plt.title(f'Joint UCB_MAX_Efficient Propagation HGPBO {nbr_repetition} Exploration')
    plt.savefig(
        f'ucb_max_efficient{folder_of_the_day}/differentiable_plots/Joint_UCB_Efficient_Neural_Propagation_HGPBO_{nbr_repetition}_Exploration_kappa_{kappa}_query_{nbr_query}')
    plt.close()


def training_procedure(nbr_query, nbr_repetition, nbr_rand_init, training_iter, k_vals, g_vals):

    current_datetime = datetime.now().strftime("%Y-%m-%d_%Hh-%Mmin-%Ss")
    current_dateday = datetime.now().strftime("%Y-%m-%d")
    workspace = f"C:/Users/preda/PycharmProjects/HGPBO/ucb_max_efficient"
    ...
    for kappa in k_vals:
        ...
        for gamma in g_vals:
                ...
                for q in tqdm(range(nbr_query)):
                    if q == 0:
                        ...
                        train_x_sub2, train_y_sub2 = random_initialization_1D(nbr_rand_init, trainsC,
                                                                        max_seen_resp_2_1D, emg=EMG, max_update=True)
                        ...
                        y_inter1 = y_mu1 + gamma * y_conf1
                        y_inter2 = y_mu2 + gamma * y_conf2

                        for i in range(len(prior_map)):
                            for j in range(len(prior_map)):
                                prior_map[i, j] = (y_inter1[i] + y_inter2[j])/2
                    ...
                    cont1 = y_mu_point_a + y_conf_point_a
                    cont2 = y_mu_point_b + y_conf_point_b

                    contribution1 = response * np.exp(cont1) / (np.exp(cont1) + np.exp(cont2))
                    contribution2 = response * np.exp(cont2) / (np.exp(cont1) + np.exp(cont2))
                    ...
                    sub1, sub1_like, train_x_sub1, train_y_sub1 = hmodel.update_model1_1D_Efficient_MAX(sub1, sub1_like, train_x_sub1,
                                                                                          train_y_sub1,
                                                                                          next_query_pins[:2],
                                                                                          response_1, max_seen_resp_1_1D,
                                                                                          training_iter=training_iter)

                    sub2, sub2_like, train_x_sub2, train_y_sub2 = hmodel.update_model1_1D_Efficient_MAX(sub2, sub2_like, train_x_sub2,
                                                                                          train_y_sub2,
                                                                                          next_query_pins[2:],
                                                                                          response_2, max_seen_resp_2_1D,
                                                                                          training_iter=training_iter)
            ...
            plt.legend()
            plt.ylim(0, 1.1)
            plt.title(f'UCB_MAX_Efficient HGP-BO {nbr_repetition} repetitions with kappa value {k} Gamma {g}')
            plt.savefig(
                f'ucb_max_efficient{folder_of_the_day}/differentiable_plots/UCB_Efficient_Prop_Neural_HGP-BO_{nbr_repetition}_repetitions_kappa_{k}_gamma_{g}_query_{nbr_query}')
            plt.close()


            vi.contour_plot_1D([sub1, sub2], test_x_1D, [test_y_1D / torch.max(test_y_1D), test_y_1D / torch.max(test_y_1D)],
                             f'/Heatmap_Neural_UCB_MAX_Efficient_HGP-BO_{nbr_repetition}_repetitions_kappa_{k}_gamma{g}_query_{nbr_query}',
                             "ucb_max_efficient", folder_of_the_day, 'N.A', neural=True)

        # Joint Section

        joint_plots(over_exploit, over_explor, kappa, g_vals, folder_of_the_day, nbr_query, nbr_repetition)
...
if __name__ == '__main__':
    ...
    k_vals = [4, 5, 6, 7, 8, 9]
    g_vals = [2.25, 2.5, 3]
    training_procedure(nbr_query, nbr_repetition, nbr_rand_init, training_iter, k_vals, g_vals)
