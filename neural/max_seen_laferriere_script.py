...
def joint_plots(joint_exploit, joint_explor, kappa, gamma, nu_vals, folder_of_the_day, nbr_query, nbr_repetition,
                model_name):
...
def joint_performance(joint_exploit, joint_explor, kappa, gamma, nu_vals, folder_of_the_day, nbr_query, nbr_repetition,
                      model_name):
    names = []
...
def run_repetition(kappa, gamma, nu, nbr_query, nbr_rand_init, training_iter, hierarchical_model, model_name,
    ...
    for q in tqdm(range(nbr_query)):
        ...
        if q == 0:
            ...
            sub1 = models.ExactGPModel(train_x_sub1, train_y_sub1, sub1_like, query_counter=sub1_qc)

            sub2_like = gpytorch.likelihoods.GaussianLikelihood()
            sub2 = models.ExactGPModel(train_x_sub2, train_y_sub2, sub2_like, query_counter=sub2_qc)
...
def training_procedure(nbr_query, nbr_repetition, nbr_rand_init, training_iter, k_vals, g_vals, nu_vals,
    ...
    current_datetime = datetime.now().strftime("%Y-%m-%d_%Hh-%Mmin-%Ss")
    current_dateday = datetime.now().strftime("%Y-%m-%d")
    workspace = os.path.join(os.path.dirname(os.path.abspath(__file__)), model_name.lower())
    ...
    for kappa in k_vals:
        ...
        for gamma in g_vals:
            for nu in nu_vals:
                ...
                plt.savefig(
                    f'{model_name.lower()}{folder_of_the_day}/differentiable_plots/{model_name}_Neural_{nbr_repetition}_reps_k_{k}_g_{g}_nu_{n}_rand_init_{nbr_rand_init}.png')
...
if __name__ == '__main__':
    ...
    for h in h_model:
        ...
        nbr_rand_init_list = np.arange(1, 11, 1)
        ...
        training_iter_list = np.arange(1, 11, 1)
        ...
        k_vals_list = np.linspace(1, 10, 10)
        ...
        g_vals_list = np.linspace(1, 10, 10)
