...
def joint_plots(joint_exploit, joint_explor, k_vals, folder_of_the_day, dimension, nbr_query, nbr_repetition,
    ...
    plt.title(f'Joint Norm_Efficient Propagation HGPBO {nbr_repetition} Exploitation eps 0,75')
    plt.savefig(
        f'{data_name}/efficient_3D{folder_of_the_day}/differentiable_plots/Joint_Norm_Efficient_Propagation_HGPBO_{nbr_repetition}_Exploitation_eps_0,75')
    plt.close()
    ...
    plt.title(f'Joint Norm_Efficient Propagation HGPBO {nbr_repetition} Exploration eps 0,75')
    plt.savefig(
        f'{data_name}/efficient_3D{folder_of_the_day}/differentiable_plots/Joint_Norm_Efficient_Propagation_HGPBO_{nbr_repetition}_Exploration_eps_0,75')
    plt.close()
    ...
    for kappa in k_vals:
        ...
        for repetition in range(nbr_repetition):
            ...
            for q in tqdm(range(nbr_query)):
                ...
                response_1, max_seen_resp_1_1D = models.update_max_seen_response_no_norm(response_1, max_seen_resp_1_1D)
                response_2, max_seen_resp_2_1D = models.update_max_seen_response_no_norm(response_2, max_seen_resp_2_1D)
                ...
                response_1, max_seen_resp_1_1D = models.update_max_seen_response_no_norm(response * contribution1, max_seen_resp_1_1D)
                response_2, max_seen_resp_2_1D = models.update_max_seen_response_no_norm(response * contribution2, max_seen_resp_2_1D)
