...
def joint_plots(joint_exploit, joint_explor, k_vals, folder_of_the_day, dimension, nbr_query, nbr_repetition,
    ...
    plt.title(f'Exploitation Score of Indep Changing HGPBO model')
    plt.legend()
    plt.ylim((0, 1.1))
    plt.savefig(
        f"{data_name}/changing_indep{folder_of_the_day}/differentiable_plots/Joint_Exploit_{data_name}_Indep_Change_HGP-BO_{nbr_repetition}_repetitions_eps0,75_dim_{dimension}.png")
    plt.close()
    ...
    plt.title(f'Exploration Score of Indep Changing HGPBO model')
    plt.legend()
    plt.ylim((0, 1.1))
    plt.savefig(
        f"{data_name}/changing_indep{folder_of_the_day}/differentiable_plots/Joint_Explor_{data_name}_Indep_Change_HGP-BO_{nbr_repetition}_repetitions_eps0,75_dim_{dimension}.png")
    plt.close()
...
def training_procedure(nbr_query, nbr_repetition, nbr_rand_init, dimension, training_iter, k_vals, data_name, data_creation_func,
    ...
    workspace = f"C:/Users/preda/PycharmProjects/HierarchicalGPBO/model_testing/{data_name}/changing_indep"
    ...
    for kappa in k_vals:
        ...
        for repetition in range(nbr_repetition):
            for q in tqdm(range(nbr_query)):
                if q == 0:
                    ...
                    master = hmodel.Changing_Data_UCB_Hierarchical_GP(train_x_hier, train_y_hier, likelihood,
                ...
                with gpytorch.settings.lazily_evaluate_kernels(state=False):
                    ...
                    sub1.set_train_data(train_x_sub1, train_y_sub1)
                    sub2.set_train_data(train_x_sub2, train_y_sub2)
            ...
            # contour_plot(master, likelihood, x_hier, y_hier, seen_x, seen_y)
        ...
        vi.contour_plot_1D(master.sub_models, x_sub1, [y_sub1, y_sub2],
                           f'/contour/Contour_{data_name}_Indep_Changing_HGP-BO_{nbr_repetition}_repetitions_eps0,75_dim_{dimension}_kappa_{kappa}',
                           'changing_indep', folder_of_the_day, data_name)
        ...
        plt.legend()
        plt.ylim(0, 1.1)
        plt.title(f'Indep_Changing HGP-BO {nbr_repetition} repetitions with kappa value {kappa}')
        plt.savefig(
            f'{data_name}/changing_indep{folder_of_the_day}/differentiable_plots/Indep_Changing_Prop_{data_name}_HGP-BO_{nbr_repetition}_repetitions_kappa_{kappa}_eps0,75')
        plt.close()

        vi.model_heatmap(heatmap_data, x_hier, y_hier,
                         f'/Heatmap_{data_name}_Indep_Changing_HGP-BO_{nbr_repetition}_repetitions_eps0,75_dim_{dimension}_kappa_{kappa}',
                         "changing_indep", folder_of_the_day, data_name)
