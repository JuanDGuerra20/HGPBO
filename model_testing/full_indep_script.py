"""
Basically an exact copy of Thomas' original model but it does not feed the training values to the submodels as a
function of the response of the hierarchical model
"""
...
def joint_plots(joint_exploit, joint_explor, k_vals, folder_of_the_day, dimension, nbr_query, nbr_repetition, data_name):
    ...
    plt.title(f'Exploitation Score of Full_Indep HGPBO model')
    plt.legend()
    plt.ylim((0, 1.1))
    plt.savefig(
        f"{data_name}/full_indep{folder_of_the_day}/differentiable_plots/Joint_Exploit_{data_name}_Full_Indep_HGP-BO_{nbr_repetition}_repetitions_eps0,75_dim_{dimension}.png")
    plt.close()
    ...
    plt.title(f'Exploration Score of Full_Indep HGPBO model')
    plt.legend()
    plt.ylim((0, 1.1))
    plt.savefig(
        f"{data_name}/full_indep{folder_of_the_day}/differentiable_plots/Joint_Explor_{data_name}_Full_Indep_HGP-BO_{nbr_repetition}_repetitions_eps0,75_dim_{dimension}.png")
    plt.close()
...
def training_procedure(nbr_query, nbr_repetition, nbr_rand_init, dimension, training_iter, k_vals, data_name, data_creation_func,
    ...
    workspace = f"C:/Users/preda/PycharmProjects/HierarchicalGPBO/model_testing/{data_name}/full_indep"
    ...
    for kappa in k_vals:
        ...
        vi.contour_plot_1D(master.sub_models, x_sub1, [y_sub1, y_sub2],
                           f'/contour/Contour_{data_name}_Full_Indep_HGP-BO_{nbr_repetition}_repetitions_eps0,75_dim_{dimension}_kappa_{kappa}',
                           'full_indep', folder_of_the_day, data_name)
        ...
        vi.model_heatmap(heatmap_data, x_hier, y_hier,
                         f'/Heatmap_{data_name}_Full_Indep_HGP-BO_{nbr_repetition}_repetitions_eps0,75_dim_{dimension}_kappa_{kappa}',
                         "full_indep", folder_of_the_day, data_name)
