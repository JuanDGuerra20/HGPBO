...
def training_procedure(nbr_query, nbr_repetition, nbr_rand_init, dimension, training_iter, k_vals, data_name, data_creation_func,
    ...
    for kappa in k_vals:
        ...
        plt.savefig(
            f'{data_name}/changing{folder_of_the_day}/differentiable_plots/Changing_Prop_{data_name}_HGP-BO_{nbr_repetition}_repetitions_kappa_{kappa}')
        plt.close()

        vi.model_heatmap(heatmap_data, x_hier, y_hier,
                         f'/Heatmap_{data_name}_Changing_HGP-BO_{nbr_repetition}_repetitions_dim_{dimension}_kappa_{kappa}',
                         "changing", folder_of_the_day, data_name)
