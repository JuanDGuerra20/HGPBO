...
def joint_plots(joint_exploit, joint_explor, k_vals, folder_of_the_day, dimension, nbr_query, nbr_repetition,
    ...
    plt.savefig(
        f'{data_name}/efficient{folder_of_the_day}/differentiable_plots/Joint_Norm_Efficient_Propagation_HGPBO_{nbr_repetition}_Exploitation_eps_0,75')
    plt.close()
    ...
    plt.savefig(
        f'{data_name}/efficient{folder_of_the_day}/differentiable_plots/Joint_Norm_Efficient_Propagation_HGPBO_{nbr_repetition}_Exploration_eps_0,75')
    plt.close()
...
def training_procedure(nbr_query, nbr_repetition, nbr_rand_init, dimension, training_iter, k_vals, data_name, data_creation_func,
    ...
    for kappa in k_vals:
        ...
        for repetition in range(nbr_repetition):
            ...
            for q in tqdm(range(nbr_query)):
                if q == 0:
                    ...
                    p1 = y_mu1 + y_conf1/(torch.sqrt(sub1_qc))
                    p2 = y_mu2 + y_conf2/(torch.sqrt(sub2_qc))
                ...
                cont1 = y_mu_point_a + kappa * torch.nan_to_num(y_conf_point_a/torch.sqrt(y_qc_a))
                cont2 = y_mu_point_b + kappa * torch.nan_to_num(y_conf_point_b/torch.sqrt(y_qc_b))
        ...
        vi.contour_plot_1D(master.sub_models, x_sub1, [y_sub1 / torch.max(y_sub1), y_sub2 / torch.max(y_sub2)],
                           f'/contour/Contour_{data_name}_Norm_Efficient_HGP-BO_{nbr_repetition}_repetitions_dim_{dimension}_kappa_{str(kappa).replace(".", ",")}',
        ...
        plt.savefig(
            f'{data_name}/efficient{folder_of_the_day}/differentiable_plots/Norm_Efficient_Prop_{data_name}_HGP-BO_{nbr_repetition}_repetitions_kappa_{k}')
        plt.close()
