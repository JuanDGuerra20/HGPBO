...
def run_repetition(kappa, gamma, nu, nbr_query, nbr_rand_init, dimension, training_iter, hierarchical_model,
                   data_creation_func, eps, model_name, folder_of_the_day, data_name, final=False, children=[], visualize=True, device="cpu"):
    x_sub1, y_sub1, x_sub2, y_sub2, x_hier, y_hier, test_x, test_x_hier = data_creation_func(dimension, eps)

    x_sub1, y_sub1, x_sub2, y_sub2, x_hier, y_hier, test_x, test_x_hier = x_sub1.to(device), y_sub1.to(device), x_sub2.to(device), y_sub2.to(device), x_hier.to(device), y_hier.to(device), test_x.to(device), test_x_hier.to(device)

    prior_map = torch.zeros(dimension, dimension, device=device)

    ground_truth_max_1 = torch.max(y_sub1).to(device)
    ground_truth_max_2 = torch.max(y_sub2).to(device)
    ground_truth_max_hier = torch.max(y_hier).to(device)

    max_seen_resp_2D = 0
    max_seen_resp_1_1D = 0
    max_seen_resp_2_1D = 0
    sub1_qc = torch.ones(x_sub1.shape, device=device)
    sub2_qc = torch.ones(x_sub2.shape, device=device)
    hier_qc = torch.ones(len(x_sub1) * len(x_sub2), device=device)
    ...
    for q in tqdm(range(nbr_query)):
        ...
        if q == 0:
            if children == []:
                ...
                train_x_sub1, train_y_sub1 = train_x_sub1.to(device), train_y_sub1.to(device)
                train_x_sub2, train_y_sub2 = train_x_sub2.to(device), train_y_sub2.to(device)

                max_seen_resp_1_1D = torch.max(train_y_sub1).to(device)
                max_seen_resp_2_1D = torch.max(train_y_sub2).to(device)
                sub1_like = gpytorch.likelihoods.GaussianLikelihood().to(device)
                sub1 = models.ExactGPModel(train_x_sub1, train_y_sub1 / max_seen_resp_1_1D, sub1_like,
                                           query_counter=sub1_qc, nu=nu).to(device)

                sub2_like = gpytorch.likelihoods.GaussianLikelihood().to(device)
                sub2 = models.ExactGPModel(train_x_sub2, train_y_sub2 / max_seen_resp_2_1D, sub2_like,
                                           query_counter=sub2_qc, nu=nu).to(device)
                ...
                sub1 = children[0].to(device)
                sub2 = children[1].to(device)

                sub1_like = sub1.likelihood.to(device)
                sub2_like = sub2.likelihood.to(device)
            ...
            master = hierarchical_model(train_x_hier, train_y_hier / max_seen_resp_2D, x_hier, likelihood,
                                        ...
                                        kappa=kappa, query_counter=hier_qc, device=device).to(device)
                ...
                if multi:
                    with mp.Pool(processes=nbr_repetition - 1) as pool:
                        ...
                        for i in range(nbr_repetition - 1):
                            ...
                            data_creation_func, eps, model_name, folder_of_the_day, data_name, False, children, visualize, device))
                            processes.append(p)
                        ...
                        try:
                            master, sub1, sub2, rep_exploration_score, rep_exploitation_score, heatmap_rep, child_1_r2, child_2_r2 = run_repetition(
                                kappa, gamma, nu, nbr_query, nbr_rand_init, dimension, training_iter, hierarchical_model,
                                data_creation_func, eps, model_name, folder_of_the_day, data_name, final=True, children=children, visualize=visualize, device=device)
                        except:
                            master, sub1, sub2, rep_exploration_score, rep_exploitation_score, heatmap_rep, child_1_r2, child_2_r2 = run_repetition(
                                ...
                                children=children, visualize=visualize, device=device)
