...
def joint_plots(joint_exploit, joint_explor, k_vals, folder_of_the_day, dimension, nbr_query, nbr_repetition,
    ...
    plt.title(f'Joint Stationary_Max_ Norm_Efficient Propagation HGPBO {nbr_repetition} Exploitation eps 0,75')
    plt.savefig(
        f'{data_name}/efficient_3D{folder_of_the_day}/differentiable_plots/Joint_Stationary_Max_Norm_Efficient_Propagation_HGPBO_{nbr_repetition}_Exploitation')
    plt.close()
    ...
    plt.title(f'Joint Stationary_Max_Norm_Efficient Propagation HGPBO {nbr_repetition} Exploration')
    plt.savefig(
        f'{data_name}/efficient_3D{folder_of_the_day}/differentiable_plots/Joint_Stationary_Max_Norm_Efficient_Propagation_HGPBO_{nbr_repetition}_Exploration')
    plt.close()
...
def training_procedure(nbr_query, nbr_repetition, nbr_rand_init, dimension, training_iter, k_vals, data_name, data_creation_func,
    ...
    workspace = f"C:/Users/preda/PycharmProjects/HierarchicalGPBO/model_testing/{data_name}/efficient_3D"
    ...
    for kappa in k_vals:
        ...
        for repetition in range(nbr_repetition):
            ...
            for q in tqdm(range(nbr_query)):
                ...
                """contribution1 = y_mu_point_a / (y_mu_point_a + y_mu_point_b)
                contribution2 = y_mu_point_b / (y_mu_point_a + y_mu_point_b)

                response_1, max_seen_resp_1_1D = models.update_max_seen_response(response * contribution1, max_seen_resp_1_1D)
                response_2, max_seen_resp_2_1D = models.update_max_seen_response(response * contribution2, max_seen_resp_2_1D)
        ...
        vi.contour_plot_1D(master.sub_models, x_sub1, [y_sub1 / torch.max(y_sub1), y_sub2 / torch.max(y_sub2)],
                           f'/contour/Contour_{data_name}_Norm_Efficient_HGP-BO_{nbr_repetition}_repetitions_eps0,75_dim_{dimension}_kappa_{str(kappa).replace(".", ",")}',
        ...
        plt.ylim(0, 1.1)
        k = str(kappa).replace('.', ',')
        plt.title(f'Stationary Max Norm_Efficient HGP-BO {nbr_repetition} repetitions with kappa value {k}')
        plt.savefig(
            f'{data_name}/efficient_3D{folder_of_the_day}/differentiable_plots/Stationary_MaxNorm_Efficient_Prop_{data_name}_HGP-BO_{nbr_repetition}_repetitions_kappa_{k}')
        plt.close()

        vi.model_heatmap(heatmap_data, x_hier, y_hier,
                         f'/Stationary_Max_Heatmap_{data_name}_Norm_Efficient_HGP-BO_{nbr_repetition}_repetitions_dim_{dimension}_kappa_{k}',
                         "efficient_3D", folder_of_the_day, data_name)
...
if __name__ == '__main__':
    ...
    nbr_repetition = 5
    nbr_rand_init = 5
    k_vals = np.array(range(12, 37, 1)) / 4
