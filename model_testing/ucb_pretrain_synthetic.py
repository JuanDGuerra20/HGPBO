...
def joint_plots(joint_exploit, joint_explor, k_vals, folder_of_the_day, dimension, nbr_query, nbr_repetition,
    ...
    plt.title(f'Joint Norm_pretrain_efficient Propagation HGPBO {nbr_repetition} Exploitation eps 0,75')
    plt.savefig(
        f'{data_name}/pretrain_efficient{folder_of_the_day}/differentiable_plots/Joint_Norm_pretrain_efficient_Propagation_HGPBO_{nbr_repetition}_Exploitation_eps_0,75')
    plt.close()
    ...
    plt.title(f'Joint Norm_pretrain_efficient Propagation HGPBO {nbr_repetition} Exploration eps 0,75')
    plt.savefig(
        f'{data_name}/pretrain_efficient{folder_of_the_day}/differentiable_plots/Joint_Norm_pretrain_efficient_Propagation_HGPBO_{nbr_repetition}_Exploration_eps_0,75')
    plt.close()


def train_submodels(q_pre, x_sub1, x_sub2, y_sub1, y_sub2):
    for q in range(q_pre):
        if q == 0:
        ...
        # Using UCB to get the next query
        next_query1 = np.argmax(upper1)
        next_query2 = np.argmax(upper2)

        q_x1, q_y1 = x_sub1[next_query1], y_sub1[next_query1]
        q_x2, q_y2 = x_sub2[next_query2], y_sub2[next_query2]

        response_1, max_seen_resp_1_1D = models.update_max_seen_response_no_norm(q_y1, max_seen_resp_1_1D)
        response_2, max_seen_resp_2_1D = models.update_max_seen_response_no_norm(q_y2, max_seen_resp_2_1D)

        sub1, sub1_like, train_x_sub1, train_y_sub1 = hmodel.update_model1_1D_max_seen(sub1, sub1_like,
                                                                                            train_x_sub1,
                                                                                            train_y_sub1,
                                                                                            q_x1,
                                                                                            response_1,
        ...
        sub2, sub2_like, train_x_sub2, train_y_sub2 = hmodel.update_model1_1D_max_seen(sub2, sub2_like,
                                                                                            train_x_sub2,
                                                                                            train_y_sub2,
                                                                                            q_x2,
                                                                                            response_2,
    ...
    return sub1, sub1_like, sub2, sub2_like, train_x_sub1, train_y_sub1, train_x_sub2, train_y_sub2, max_seen_resp_1_1D, max_seen_resp_2_1D


def training_procedure(nbr_query, nbr_repetition, nbr_rand_init, dimension, training_iter, k_vals, data_name,
                       data_creation_func,
                       eps):
    ...
    for kappa in k_vals:
        ...
        for repetition in range(nbr_repetition):
            ...
            for q in tqdm(range(nbr_query)):
                if q == 0:
                    # Need to initialize the model - Will be random in this method
                    sub1, sub1_like, sub2, sub2_like, train_x_sub1, train_y_sub1, train_x_sub2, train_y_sub2, max_seen_resp_1_1D, max_seen_resp_2_1D = train_submodels(
                        40, x_sub1, x_sub2, y_sub1, y_sub2)
                    ...
                    _, y_conf1 = observed_pred1.confidence_region()
                    _, y_conf2 = observed_pred2.confidence_region()

                    for i in range(len(prior_map)):
                        for j in range(len(prior_map)):
                            prior_map[i, j] = (y_conf1[i] + y_conf2[j]) / 2

                    prior_map_max = torch.max(prior_map)

                    prior_hierarchical_kernel = hmodel.hierarchical_kernel("add_kernel", sub1, sub2)
                    likelihood = gpytorch.likelihoods.GaussianLikelihood()
                    master = hmodel.Efficient_UCB_Hierarchical_GP(train_x_hier, train_y_hier / max_seen_resp_2D,
                                                                  likelihood,
                ...
                cont1 = y_mu_point_a + kappa * torch.nan_to_num(torch.sqrt(y_conf_point_a))
                cont2 = y_mu_point_b + kappa * torch.nan_to_num(torch.sqrt(y_conf_point_b))
        ...
        vi.contour_plot_1D(master.sub_models, x_sub1, [y_sub1 / torch.max(y_sub1), y_sub2 / torch.max(y_sub2)],
                           f'/contour/Contour_{data_name}_Norm_pretrain_efficient_HGP-BO_{nbr_repetition}_repetitions_dim_{dimension}_kappa_{str(kappa).replace(".", ",")}',
                           'pretrain_efficient', folder_of_the_day, data_name)
        exploration_scores = []
        exploitation_scores = []
        ...
        plt.title(f'Norm_pretrain_efficient HGP-BO {nbr_repetition} repetitions with kappa value {k}')
        plt.savefig(
            f'{data_name}/pretrain_efficient{folder_of_the_day}/differentiable_plots/Norm_pretrain_efficient_Prop_{data_name}_HGP-BO_{nbr_repetition}_repetitions_kappa_{k}')
        plt.close()

        vi.model_heatmap(heatmap_data, x_hier, y_hier,
                         f'/Heatmap_{data_name}_Norm_pretrain_efficient_HGP-BO_{nbr_repetition}_repetitions_dim_{dimension}_kappa_{k}',
                         "pretrain_efficient", folder_of_the_day, data_name)
...
if __name__ == '__main__':
    ...
    for dataset_num in [2]:
        ...
        training_procedure(nbr_query, nbr_repetition, nbr_rand_init, dimension, training_iter, k_vals, data_name,
                           data_creation_func,
                           eps)
