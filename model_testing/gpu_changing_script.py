...
if __name__ == '__main__':
    ...
    for kappa in ks:
        ...
        for repetition in range(nbr_repetition):
            for q in tqdm(range(nbr_query)):
                if q == 0:
                    ...
                    train_x_hier, train_y_hier = hierarchical_select_random_queries(nbr_rand_init, x_hier_cuda, y_hier_cuda)
                    ...
                    """train_x_sub1 = train_x_sub1.to(device)
                    train_x_sub2 = train_x_sub2.to(device)
                    train_y_sub1 = train_y_sub1.to(device)
                    train_y_sub2 = train_y_sub2.to(device)"""
                    sub1 = sub1.to(device)
                    sub2 = sub2.to(device)
                    ...
                    master = hgpu.GPU_Changing_Data_UCB_Hierarchical_GP(train_x_hier, train_y_hier, likelihood,
                                                                      prior_hierarchical_kernel,
                                                                      prior_map / prior_map_max, kernel_op='add_kernel',
                                                                      sub_models=[sub1, sub2],
                                                                      kappa=kappa, device=device)

                    """train_x_hier_cuda = train_x_hier.to(device)
                    train_y_hier_cuda = train_y_hier.to(device)"""
                    ...
                    with gpytorch.settings.lazily_evaluate_kernels(state=False):
                        observed_pred = hmodel.make_Hierarchique_prediction(master, test_x_hier_cuda, likelihood, device=device)
            ...
            pred = hmodel.make_Hierarchique_prediction(master, test_x_hier_cuda, master.likelihood, device=device)
        ...
        vi.contour_plot_1D(master.sub_models, x_sub1, [y_sub1, y_sub2],
                           f'/contour/Contour_{data_name}_Changing_HGP-BO_{nbr_repetition}_repetitions_eps0,75_dim_{dimension}_kappa_{kappa}',
                           'changing', folder_of_the_day, data_name)
        ...
        plt.legend()
        plt.ylim(0, 1.1)
        plt.title(f'Changing HGP-BO {nbr_repetition} repetitions with kappa value {kappa}')
        plt.savefig(
            f'{data_name}/changing{folder_of_the_day}/differentiable_plots/Changing_Prop_{data_name}_HGP-BO_{nbr_repetition}_repetitions_kappa_{kappa}_eps0,75')
        plt.close()

        vi.model_heatmap(heatmap_data, x_hier, y_hier,
                         f'/Heatmap_{data_name}_Changing_HGP-BO_{nbr_repetition}_repetitions_eps0,75_dim_{dimension}_kappa_{kappa}',
                         "changing", folder_of_the_day, data_name)

    joint_plots(over_exploit, over_explor, ks, folder_of_the_day, dimension, nbr_query, nbr_repetition, data_name)"""
